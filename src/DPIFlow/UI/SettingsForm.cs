...
namespace DPIFlow.UI
{
    internal sealed class SettingsForm : Form
    {
        ...
        internal SettingsForm(AppSettings settings, List<MonitorInfo> monitors, Func<WindowInfo> lastWindow, Action<AppSettings> save)
        {
            ...
            _save = save;
            _rules = new BindingList<ApplicationRule>(settings.Rules.Select(CloneRule).ToList());
            Text = "DPIFlow Settings";
        ...
        private static ApplicationRule CloneRule(ApplicationRule source)
        {
            return new ApplicationRule
            {
                ...
                MonitorMatch = source.MonitorMatch,
        ...
        private void BuildUi()
        {
            ...
            _enabled.Text = "Enable automation";
            ...
            top.Controls.Add(new Label { AutoSize = true, Text = "  Windows handles normal Per-Monitor DPI; rules are only for apps that need extra per-display behavior." });
            root.Controls.Add(top, 0, 0);

            var monitorList = new ListView { Dock = DockStyle.Fill, View = View.Details, FullRowSelect = true };
            monitorList.Columns.Add("Display", 110);
            monitorList.Columns.Add("Name", 180);
            monitorList.Columns.Add("Match key", 130);
            monitorList.Columns.Add("DPI / scale", 110);
            monitorList.Columns.Add("Bounds", 220);
            foreach (var m in _monitors)
            {
                monitorList.Items.Add(new ListViewItem(new[]
                {
                    m.DeviceName + (m.Primary ? " *" : ""),
                    string.IsNullOrWhiteSpace(m.FriendlyName) ? "(unnamed)" : m.FriendlyName,
                    m.MatchKey,
                    m.DpiX + " / " + m.ScalePercent + "%",
                    m.Bounds.ToString()
                }));
            }
            ...
            var quick = new Button { Text = "Quick-add last app", AutoSize = true };
            quick.Click += (s, e) => QuickAdd();
            var blank = new Button { Text = "Add blank rule", AutoSize = true };
            blank.Click += (s, e) => _rules.Add(new ApplicationRule());
            var remove = new Button { Text = "Remove selected", AutoSize = true };
            remove.Click += (s, e) => RemoveSelected();
            buttons.Controls.Add(quick);
            buttons.Controls.Add(blank);
            ...
            var save = new Button { Text = "Save && Close", AutoSize = true };
        ...
        private void ConfigureGrid()
        {
            ...
            adapter.Items.AddRange("Observe", "ChromiumKeyboardZoom");
            _grid.Columns.Add(adapter);
        ...
        private void QuickAdd()
        {
            ...
            if (window == null)
            {
                MessageBox.Show("Use the target application, then open DPIFlow settings from the tray. DPIFlow remembers the last foreground app.", "DPIFlow");
                return;
            }
            ...
            foreach (var monitor in _monitors)
            {
                ...
                _rules.Add(new ApplicationRule
                {
                    ...
                    MonitorMatch = monitor.MatchKey,
                    Adapter = chromium ? "ChromiumKeyboardZoom" : "Observe",
        ...
        private void SaveAndClose()
        {
            ...
            _settings.Rules = _rules.Select(CloneRule).ToList();
            _save(_settings);
